...
% Your job is to first sort 'valid_wavelet_coefficients' based on the
% modulus values and then find a threshhold. 
...
% %  first we sort the valid_wavelet_coeff vector in descending order
% % then we multiply percent in lenght of this vector to achieve  the index
% % number of coefficent which has the threshold value
% % so with this algorithm we have Percent percent of coeffs which are
% % greater than threshold

    sorted_coeff  = sort( valid_wavelet_coeff , 'descend');
    threshold       = sorted_coeff(ceil( percent.*size(sorted_coeff , 1)));
...
% than threshhold and bigger than min_e
